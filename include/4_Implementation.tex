...
have internal dependencies themselves, commonly described as \say{Dependency Hell}: %TODO: cite https://en.wikipedia.org/wiki/Dependency_hell
...
% TODO: some closing remarks about JDBC not actually abstracting databases good, since they all support different subsets of SQL
% Type safety shifting errors to the compilation process instead of testing, detecting bugs as early as possible

\subsection*{Database Migrations}
% TODO: previous migrations: none
...
% TODO: summarize auth + user information process
% auth process: regular LDAP authentication with ApacheDS, using username / password on TUM active directory
% user information process: user fields according to RFC4519
% Email addresses NOT according to RFC, but proprietary microfost way.
% documented here: https://support.microsoft.com/en-us/help/3190357/
% primary mail addresses are stored in 'mail' field
% additionally in the multi-value 'proxyAddesses' attribute, prefixed with uppercase 'SMTP:'
% secondary email addresses, are contained also contained in this attribute, but prefixed with lowercase 'smtp:'
% historically, this field could also contain X.400, X.500, cc:Mail, MS-Mail, GroupWise and other obsolete mail
% addresses, thus the field is prefixed with the corresponding protocol.
% None of those protocoly are actively in use or supported, thus we chose to ignore those and assume a SMTP only
% configuration
%
% all mail addresses are relevant for email certificates, so correctly extracting them is important
